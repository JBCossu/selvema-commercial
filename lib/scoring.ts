...
export async function scoreConversation(
  ...
  const system = `Tu es analyste commercial pour une agence immobilière. On te donne l'intégralité d'une conversation entre un visiteur et l'assistant en ligne de l'agence, ainsi que la base de connaissances de l'agence.

Évalue le potentiel commercial du visiteur en appelant l'outil \`attribuer_score\`. Sois strict et factuel : ne coche un critère que si l'information est réellement présente dans la conversation. En cas de doute, coche false.
...
${knowledgeBasePrompt(client)}`;
  ...
  const score = (Object.values(crits) as { points: number }[]).reduce(
    (s, c) => s + c.points,
    0
  );
